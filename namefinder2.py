...
def filterNames (possiblenames):
        ...
        femaleNames = makeList("./texts/fullnamesInTextfemalenames.txt")
        maleNames = makeList("./texts/malenames.txt")
        validlasts = makeList("./texts/surnames.txt")
        validfirsts = femaleNames + maleNames
        #our lists of "real" names

        for fullname in justthenames:
                if  nameCheck(fullname, validfirsts, validlasts):
                # checks for duplicates and references database
                        if (not fullname in validnames.keys()):
                                validnames[justthenames.index(fullname)]=[justthenames.index(fullname),1]
                        else:
                                validnames[justthenames.index(fullname)][1]+=1  
...
#call the function findNames(filename) and it will return a list of the names
